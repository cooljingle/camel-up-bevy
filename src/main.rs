...
use game::ai::{AiConfig, AiThinkTimer, ai_decision_system};
use systems::setup::{setup_game, cleanup_game, initial_roll_animation_system};
use systems::movement::{MoveCamelEvent, MoveCrazyCamelEvent, MovementCompleteEvent, move_camel_system, move_crazy_camel_system};
use systems::turn::{
    TurnState, TakeLegBetAction, PlaceDesertTileAction, RollPyramidAction, PlaceRaceBetAction,
    PyramidRollResult, CrazyCamelRollResult, PlayerLegBetsStore, PlayerPyramidTokens,
    handle_leg_bet_action, handle_pyramid_roll_action, handle_race_bet_action, handle_desert_tile_action,
    advance_turn_system, check_leg_end_system, check_game_end_system,
};
use systems::leg::calculate_final_scores;
use systems::animation::{animate_movement_system, animate_multi_step_movement_system, fade_out_system, dice_result_popup_system, dice_roll_animation_system, particle_system, firework_system, explosion_particle_system, crown_drop_system};
use ui::main_menu::main_menu_ui;
use ui::hud::{game_hud_ui, update_ui_on_roll, update_ui_on_crazy_roll, update_dice_popup_timer, update_camel_position_animations, leg_scoring_modal_ui, UiState, CamelPositionAnimations};
...
fn main() {
    ...
    app.add_plugins(DefaultPlugins.set(WindowPlugin {
        ...
        .add_message::<PlaceDesertTileAction>()
    ...
    // UI scaling system - runs every frame to handle window resizing
    app.add_systems(Update, scale_ui_to_fit);
    ...
    app.add_systems(OnEnter(GameState::Playing), setup_game_with_resources)
        ...
        .add_systems(EguiPrimaryContextPass, game_hud_ui.run_if(in_state(GameState::Playing)))
        .add_systems(EguiPrimaryContextPass, leg_scoring_modal_ui.run_if(in_state(GameState::Playing)))
        .add_systems(EguiPrimaryContextPass, game_end_ui.run_if(in_state(GameState::GameEnd)))
        // Game logic systems (Update schedule)
        .add_systems(Update, (
            handle_leg_bet_action,
            handle_pyramid_roll_action,
            handle_race_bet_action,
            handle_desert_tile_action,
        ).run_if(in_state(GameState::Playing)))
        .add_systems(Update, move_camel_system.run_if(in_state(GameState::Playing)))
        .add_systems(Update, move_crazy_camel_system.run_if(in_state(GameState::Playing)))
        .add_systems(Update, update_ui_on_roll.run_if(in_state(GameState::Playing)))
        .add_systems(Update, update_ui_on_crazy_roll.run_if(in_state(GameState::Playing)))
        .add_systems(Update, update_dice_popup_timer.run_if(in_state(GameState::Playing)))
        .add_systems(Update, update_camel_position_animations.run_if(in_state(GameState::Playing)))
        ...
        .add_systems(Update, advance_turn_system.run_if(in_state(GameState::Playing)))
        .add_systems(Update, check_leg_end_system.run_if(in_state(GameState::Playing)))
        .add_systems(Update, check_game_end_system.run_if(in_state(GameState::Playing)))
        // Initial roll animation system
        .add_systems(Update, initial_roll_animation_system.run_if(in_state(GameState::Playing)))
        ...
        // Game end scoring
        .add_systems(OnEnter(GameState::GameEnd), (calculate_final_scores, setup_game_end_state))
        // Cleanup when returning to main menu
        .add_systems(OnEnter(GameState::MainMenu), cleanup_game)
        .run();
}

// Design resolution - the game is designed for this size
...
/// This makes the UI responsive - scaling up/down to fit different screen sizes
fn scale_ui_to_fit(
    ...
    // Use the smaller scale to ensure UI fits in both dimensions
    // This ensures content never overflows the viewport
    let scale = scale_x.min(scale_y);

    // Apply a small margin to ensure nothing is clipped at edges (5% reduction)
...
fn set_window_icon(_: NonSendMarker) {
    ...
    let icon = load_icon_from_file("assets/icon.png")
        .or_else(|| {
...
fn create_camel_icon() -> Option<Icon> {
    ...
    for y in 0..SIZE {
        for x in 0..SIZE {
            ...
            let dist_to_base = if fy >= base_y - 1.0 && fy <= base_y + 1.0 { 0.0 } else { 2.0 };
            let near_edge = dist_to_left_edge < 1.2 || dist_to_right_edge < 1.2 || dist_to_base < 1.0;
